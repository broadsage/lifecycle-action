...
export class EolAnalyzer {
    ...
    analyzeProductCycle(
        ...
        const releaseDate = cycle.releaseDate
            ? parseISO(cycle.releaseDate)
            : null;
    ...
    async analyzeProduct(
        ...
        try {
            ...
            core.error(`Failed to analyze product ${product}: ${getErrorMessage(error)}`);
            throw error;
        }
    }
    ...
    async analyzeProducts(
        ...
        for (const product of products) {
            try {
                ...
                core.warning(`Skipping product ${product} due to error: ${getErrorMessage(error)}`);
            }
        }
