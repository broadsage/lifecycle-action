...
export function formatAsMarkdown(results: ActionResults): string {
    ...
    if (results.eolProducts.length === 0 && results.approachingEolProducts.length === 0) {
 ...
 */
export async function writeToStepSummary(results: ActionResults): Promise<void> {
...
export async function writeToFile(
    ...
    try {
        ...
        core.error(`Failed to write to file ${filePath}: ${getErrorMessage(error)}`);
        throw error;
    }
}
